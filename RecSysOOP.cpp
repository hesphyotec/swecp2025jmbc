...
class CreateWord2Vec {// Creates vectors
	...
	void start() {//super easy way to use this class
			...
			sqlite3_exec(db, "SELECT id, name FROM Ingredients WHERE vectors IS NULL", callback, &results, &errmsg);
			getResults(results);
			std::cout << "Done! \n";
			sqlite3_close(db);
		}
...
public:
	...
	double ingredientToVector(const std::vector<int>& ingredientID) {
			...
			if (ingredientID.empty()) { return 0.0; }
			sqlite3_stmt *stmt;
			double count = 0.0;
...
class Recommend {
	...
	recommendVec euclidean (double searchedVector, pairVec toSearch) {//this returns a list of
			...
			for (const auto &[id,img] : toSearch) {
				...
				results.emplace_back(std::tuple(id, img, dist));
